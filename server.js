...
const JWT_SECRET = process.env.JWT_SECRET || 'nebula-secret-key';
...
app.get('/api/products/search/:query', async (req, res) => {
    try {
        ...
        const products = await Product.find({ $and: ors }).limit(50);
        res.json(products);
    } catch (error) {
        res.status(500).json({ message: 'Server error', error: error.message });
    }
...
app.post('/api/cart/add', authenticateToken, async (req, res) => {
    try {
        ...
        // For now, we'll return success and let frontend handle local storage
        res.json({ 
...
app.get('/api/cart', authenticateToken, async (req, res) => {
    try {
        // In a real app, fetch from database/Redis
...
app.get('/api/user/preferences', authenticateToken, async (req, res) => {
    try {
        // In a real app, fetch from user document
...
app.put('/api/user/preferences', authenticateToken, async (req, res) => {
    try {
        const { theme, reduceAnim, language } = req.body;
        
        // In a real app, update user document
        // For now, just return success
        res.json({ 
            message: 'Preferences updated',
            preferences: { theme, reduceAnim, language }
        });
    } catch (error) {
        res.status(500).json({ message: 'Server error', error: error.message });
    }
