...
const closeButtons = {
    ...
    wishlist: document.getElementById('closeWishlist')
};
...
async function initializeThemeSwitcher() {
    try {
        ...
        let savedTheme = localStorage.getItem('va_theme') || 'volt';
        let reduce = localStorage.getItem('va_reduce_anim') === 'true';

        // If logged in, try to load preferences from backend and sync locally
        if (hasToken) {
            try {
                ...
                if (prefs) {
                    if (prefs.theme) {
                        ...
                        localStorage.setItem('va_theme', savedTheme);
                    }
                    if (typeof prefs.reduceAnim === 'boolean') {
                        reduce = prefs.reduceAnim;
                        localStorage.setItem('va_reduce_anim', String(reduce));
                    }
                }
            } catch (e) {
                console.warn('Failed to load theme prefs from backend, using localStorage');
            }
        }

        root.setAttribute('data-theme', savedTheme);
        if (reduce) root.setAttribute('data-reduce-anim', 'true');
        updateActiveThemeChip(savedTheme);
        ...
        document.querySelectorAll('.theme-chip').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (hasToken) {
                    saveUserPreferences({ theme }).catch(() => {});
                }
                showNotification(`Theme changed to ${btn.textContent}`, 'success');
            });
        });
        ...
        if (toggle) {
            ...
            toggle.addEventListener('change', () => {
                ...
                if (enabled) root.setAttribute('data-reduce-anim', 'true');
                else root.removeAttribute('data-reduce-anim');
                localStorage.setItem('va_reduce_anim', String(enabled));
                if (hasToken) {
                    saveUserPreferences({ reduceAnim: enabled }).catch(() => {});
                }
            });
        }

        // When clicking the palette button, toggle dropdown in touch devices
        const themeBtn = document.getElementById('themeBtn');
        const themeMenu = themeBtn ? themeBtn.closest('.theme-menu') : null;
        ...
        console.warn('Theme switcher init failed', e);
    }
}

function updateActiveThemeChip(theme) {
    try {
...
function setupEventListeners() {
    ...
    // ensure click works on mobile too (prevent default form submits)
    elements.searchBtn.addEventListener('mousedown', (e)=> e.preventDefault());
...
async function loadProducts() {
    try {
        ...
        console.warn('Failed to load products from backend, using static data:', error);
...
async function handleSearch() {
    ...
    try {
        // local-first filtering for instant feedback
        ...
        // also fetch server results to refine (will replace when received)
        const results = await apiCall(`/products/search/${encodeURIComponent(query)}`);
        if (Array.isArray(results) && results.length) {
            displayProducts(results);
        }
    } catch (error) {
        console.warn('Search failed, using local results:', error);
...
async function filterByCategory(category) {
    try {
        const results = await apiCall(`/products/category/${encodeURIComponent(category)}`);
        displayProducts(results);
    } catch (error) {
        console.warn('Category filter failed, using local results:', error);
...
async function updateQuantity(productId, newQuantity) {
    try {
        ...
        console.warn('Backend cart update failed, using local storage:', error);
        updateQuantityLocal(productId, newQuantity);
    }
}
...
async function clearCart() {
    try {
        ...
        console.warn('Backend cart clear failed:', error);
    }
...
async function toggleWishlist(productId) {
    ...
    // If wishlist modal is open, refresh its contents
    if (modals.wishlist && modals.wishlist.style.display === 'block') {
...
function displayWishlistItems() {
    ...
    if (wishlist.length === 0) {
        container.innerHTML = `
            <div style="text-align: center; padding: 2rem;">
                ...
                <button class="cta-btn primary" onclick="closeModal(modals.wishlist)">Start Exploring</button>
            </div>
        `;
        return;
    }
    ...
    wishlist.forEach(product => {
        ...
        div.innerHTML = `
            <img src="${product.image}" alt="${product.name}" onerror="this.src='https://via.placeholder.com/60x60/f0f0f0/666?text=Product'">
            <div class="cart-item-info">
                <div class="cart-item-title">${product.name}</div>
                <div class="cart-item-price">₹${product.price.toLocaleString()}</div>
            </div>
            <div class="cart-item-quantity">
                <button class="quantity-btn" onclick="addToCart('${product._id}')">Add to Cart</button>
                <button class="quantity-btn" onclick="toggleWishlist('${product._id}')">Remove</button>
            </div>
        `;
        ...
        try {
            ...
            if (prefs) {
                if (prefs.theme) {
                    ...
                    localStorage.setItem('va_theme', prefs.theme);
                    updateActiveThemeChip(prefs.theme);
                }
                if (typeof prefs.reduceAnim === 'boolean') {
                    if (prefs.reduceAnim) {
                        document.documentElement.setAttribute('data-reduce-anim', 'true');
                    } else {
                        document.documentElement.removeAttribute('data-reduce-anim');
                    }
                    localStorage.setItem('va_reduce_anim', String(prefs.reduceAnim));
                    const toggle = document.getElementById('animToggle');
                    if (toggle) toggle.checked = prefs.reduceAnim;
                }
            }
        } catch (e) {
...
function buyNow(productId) {
    ...
    if (product) {
        ...
        closeModal(modals.product);
        openModal(modals.cart);
        showNotification(`Ready to checkout: ${product.name}`, 'success');
    }
}
...
function showNotification(message, type = 'info') {
    ...
    const colors = { 
        success: 'linear-gradient(135deg, #21e1b6, #7c5cff)', 
        error: 'linear-gradient(135deg, #ff6b6b, #ff8b8b)', 
        warning: 'linear-gradient(135deg, #ffd166, #ffb347)', 
        info: 'linear-gradient(135deg, #7c5cff, #21e1b6)' 
    };
...
function setupHeroButtons() {
    // Shop Now button
    const shopNowBtn = document.querySelector('.hero-section .cta-btn.primary');
    if (shopNowBtn) {
    ...
    // View Deals button
    const viewDealsBtn = document.querySelector('.hero-section .cta-btn.secondary');
    if (viewDealsBtn) {
    ...
    // Other hero buttons
    const heroButtons = document.querySelectorAll('.hero-slide .cta-btn');
